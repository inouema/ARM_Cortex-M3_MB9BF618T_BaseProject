...
/*!
	...
	@date           2012.05.05
	@brief          Based on
	                Copyright (C) 2012 FUJITSU SEMICONDUCTOR LIMITED Sample UART.
...
static uint8_t     f_rx_buf[1];
static uint8_t     f_tx_buf[1];
...
static void InitPort(void)
{
    ...
    */
	...
	FM3_GPIO->PFR4   = FM3_GPIO->PFR4   | 0x00000300; /* P48,P49を周辺機能端子として使用する (bit8,bit9) */
...
void putch(uint8_t data)
{
	...
	f_tx_buf[0] = data;
#if 0
	uart->BufTx(&f_tx_buf[0], &size, UartDev_FLAG_BLOCKING);
#else
	do {
		uart->BufTx(&f_tx_buf[0], &size, UartDev_FLAG_NONBLOCKING);
		if( size == 1 ) return;
    } while(1);
#endif
}
